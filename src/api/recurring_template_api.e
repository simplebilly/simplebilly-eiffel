...
feature -- API Access
	...
	create_recurring_template (body: ANY): detachable RECURRING_TEMPLATE
		...
		do
			...
			l_path := "/api/v1/recurring-templates"
			...
			elseif attached { RECURRING_TEMPLATE } l_response.data ({ RECURRING_TEMPLATE }) as l_data then
				Result := l_data
			else
	...
	delete_recurring_template (template_id: STRING_32)
		...
		do
			...
			l_path := "/api/v1/recurring-templates/{template_id}"
	...
	list_recurring_templates : detachable LIST [RECURRING_TEMPLATE]
			...
			-- 
			-- 
			-- Result LIST [RECURRING_TEMPLATE]
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [RECURRING_TEMPLATE] } l_response.data ({ LIST [RECURRING_TEMPLATE] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
