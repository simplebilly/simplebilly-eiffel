...
feature -- API Access
	...
	absence (id: UUID): detachable ABSENCE
		...
		do
			...
			elseif attached { ABSENCE } l_response.data ({ ABSENCE }) as l_data then
				Result := l_data
			else
	...
	absences (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [ABSENCE]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [ABSENCE] } l_response.data ({ LIST [ABSENCE] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	create_absence (absence_create: ABSENCE_CREATE): detachable ABSENCE
			...
			-- 
			-- 
			-- argument: absence_create  (required)
			-- 
			-- 
			-- Result ABSENCE
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(absence_create)
			l_path := "/api/v1/absences"
	...
	update_absence (id: UUID; absence_update: ABSENCE_UPDATE): detachable ABSENCE
			...
			-- argument: id  (required)
			-- 
			-- argument: absence_update  (required)
			-- 
			-- 
			-- Result ABSENCE
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(absence_update)
			l_path := "/api/v1/absences/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
