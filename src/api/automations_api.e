...
feature -- API Access
	...
	list_automations : detachable LIST [AUTOMATION_DTO]
		...
		do
			...
			elseif attached { LIST [AUTOMATION_DTO] } l_response.data ({ LIST [AUTOMATION_DTO] }) as l_data then
				Result := l_data
			else
	...
	trigger_automation (key: STRING_32): detachable ANY
		...
		do
			...
			l_path := "/api/v1/automations/{key}/trigger"
	...
	update_automation (key: STRING_32; update_automation: UPDATE_AUTOMATION): detachable AUTOMATION_DTO
			...
			-- argument: key  (required)
			-- 
			-- argument: update_automation  (required)
			-- 
			-- 
			-- Result AUTOMATION_DTO
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			reset_error
			create l_request
			l_request.set_body(update_automation)
			l_path := "/api/v1/automations/{key}"
			l_path.replace_substring_all ("{"+"key"+"}", api_client.url_encode (key.out))
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { AUTOMATION_DTO } l_response.data ({ AUTOMATION_DTO }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
