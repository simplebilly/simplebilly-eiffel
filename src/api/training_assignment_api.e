...
feature -- API Access
	...
	create_training_assignment (training_assignment_create: TRAINING_ASSIGNMENT_CREATE): detachable TRAINING_ASSIGNMENT
			...
			-- 
			-- 
			-- argument: training_assignment_create  (required)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(training_assignment_create)
			l_path := "/api/v1/training-assignments"
			...
			elseif attached { TRAINING_ASSIGNMENT } l_response.data ({ TRAINING_ASSIGNMENT }) as l_data then
				Result := l_data
			else
	...
	delete_training_assignment (id: UUID)
		...
		do
			...
			l_path := "/api/v1/training-assignments/{id}"
	...
	training_assignments (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [TRAINING_ASSIGNMENT]
			-- 
			-- 
			-- 
			...
			-- 
			-- 
			-- Result LIST [TRAINING_ASSIGNMENT]
		require
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [TRAINING_ASSIGNMENT] } l_response.data ({ LIST [TRAINING_ASSIGNMENT] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_training_assignment (id: UUID; training_assignment_update: TRAINING_ASSIGNMENT_UPDATE): detachable TRAINING_ASSIGNMENT
			...
			-- argument: id  (required)
			-- 
			-- argument: training_assignment_update  (required)
			-- 
			-- 
			-- Result TRAINING_ASSIGNMENT
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(training_assignment_update)
			l_path := "/api/v1/training-assignments/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
