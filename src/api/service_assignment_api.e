...
feature -- API Access
	...
	create_service_assignment (service_assignment_create: SERVICE_ASSIGNMENT_CREATE): detachable SERVICE_ASSIGNMENT
			...
			-- 
			-- 
			-- argument: service_assignment_create  (required)
			-- 
			-- 
			-- Result SERVICE_ASSIGNMENT
		require
		local
  			l_path: STRING
		...
		do
			reset_error
			create l_request
			l_request.set_body(service_assignment_create)
			l_path := "/api/v1/service-assignments"
			...
			elseif attached { SERVICE_ASSIGNMENT } l_response.data ({ SERVICE_ASSIGNMENT }) as l_data then
				Result := l_data
			else
	...
	delete_service_assignment (id: UUID)
		...
		do
			...
			l_path := "/api/v1/service-assignments/{id}"
	...
	service_assignments (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [SERVICE_ASSIGNMENT]
			-- 
			-- 
			-- 
			...
			-- 
			-- 
			-- Result LIST [SERVICE_ASSIGNMENT]
		require
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [SERVICE_ASSIGNMENT] } l_response.data ({ LIST [SERVICE_ASSIGNMENT] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_service_assignment (id: UUID; service_assignment_update: SERVICE_ASSIGNMENT_UPDATE): detachable SERVICE_ASSIGNMENT
			...
			-- argument: id  (required)
			-- 
			-- argument: service_assignment_update  (required)
			-- 
			-- 
			-- Result SERVICE_ASSIGNMENT
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(service_assignment_update)
			l_path := "/api/v1/service-assignments/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
