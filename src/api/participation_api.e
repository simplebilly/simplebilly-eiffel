...
feature -- API Access
	...
	create_participation (participation_create: PARTICIPATION_CREATE): detachable PARTICIPATION
			...
			-- 
			-- 
			-- argument: participation_create  (required)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(participation_create)
			l_path := "/api/v1/participations"
			...
			elseif attached { PARTICIPATION } l_response.data ({ PARTICIPATION }) as l_data then
				Result := l_data
			else
	...
	delete_participation (id: UUID)
		...
		do
			...
			l_path := "/api/v1/participations/{id}"
	...
	participations (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [PARTICIPATION]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [PARTICIPATION] } l_response.data ({ LIST [PARTICIPATION] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_participation (id: UUID; participation_update: PARTICIPATION_UPDATE): detachable PARTICIPATION
			...
			-- argument: id  (required)
			-- 
			-- argument: participation_update  (required)
			-- 
			-- 
			-- Result PARTICIPATION
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(participation_update)
			l_path := "/api/v1/participations/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
