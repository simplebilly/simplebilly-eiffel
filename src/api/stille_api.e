...
feature -- API Access
	...
	stille_export_api (year: INTEGER_32): detachable STILLE_EXPORT_RESPONSE
		...
		do
			...
			l_path := "/api/v1/bookkeeping/stille/export"
			...
			elseif attached { STILLE_EXPORT_RESPONSE } l_response.data ({ STILLE_EXPORT_RESPONSE }) as l_data then
				Result := l_data
			else
	...
	stille_report_api (year: INTEGER_32): detachable STILLE_REPORT
		...
		do
			...
			l_path := "/api/v1/bookkeeping/stille/report"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "year", year));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { STILLE_REPORT } l_response.data ({ STILLE_REPORT }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
