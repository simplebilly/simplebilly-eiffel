...
feature -- API Access
	...
	create_service_job (service_job_create: SERVICE_JOB_CREATE): detachable SERVICE_JOB
			...
			-- 
			-- 
			-- argument: service_job_create  (required)
			-- 
			-- 
			-- Result SERVICE_JOB
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(service_job_create)
			l_path := "/api/v1/service-jobs"
			...
			elseif attached { SERVICE_JOB } l_response.data ({ SERVICE_JOB }) as l_data then
				Result := l_data
			else
	...
	service_jobs (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [SERVICE_JOB]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [SERVICE_JOB] } l_response.data ({ LIST [SERVICE_JOB] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_service_job (id: UUID; service_job_update: SERVICE_JOB_UPDATE): detachable SERVICE_JOB
			...
			-- argument: id  (required)
			-- 
			-- argument: service_job_update  (required)
			-- 
			-- 
			-- Result SERVICE_JOB
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(service_job_update)
			l_path := "/api/v1/service-jobs/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
