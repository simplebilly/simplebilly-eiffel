...
feature -- API Access
	...
	create_delivery_note (delivery_note_create: DELIVERY_NOTE_CREATE): detachable DELIVERY_NOTE
			...
			-- 
			-- 
			-- argument: delivery_note_create  (required)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(delivery_note_create)
			l_path := "/api/v1/delivery-notes"
	...
	delete_delivery_note (delivery_note_id: STRING_32)
			...
			-- 
			-- 
			-- argument: delivery_note_id  (required)
			-- 
			-- 
		require
		local
		...
		do
			...
			l_path := "/api/v1/delivery-notes/{delivery_note_id}"
			l_path.replace_substring_all ("{"+"delivery_note_id"+"}", api_client.url_encode (delivery_note_id.out))
	...
	deliverynote_restore (delivery_note_id: STRING_32): detachable DELIVERY_NOTE
		...
		do
			...
			l_path := "/api/v1/delivery-notes/{delivery_note_id}/restore"
			l_path.replace_substring_all ("{"+"delivery_note_id"+"}", api_client.url_encode (delivery_note_id.out))
	...
	download_delivery_note_pdf (delivery_note_id: STRING_32)
		...
		do
			...
			l_path := "/api/v1/delivery-notes/{delivery_note_id}/pdf"
			l_path.replace_substring_all ("{"+"delivery_note_id"+"}", api_client.url_encode (delivery_note_id.out))
	...
	list_delivery_notes (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [DELIVERY_NOTE]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [DELIVERY_NOTE] } l_response.data ({ LIST [DELIVERY_NOTE] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	pursue_delivery_note (delivery_note_id: STRING_32): detachable INVOICE
		...
		do
			...
			l_path := "/api/v1/delivery-notes/{delivery_note_id}/pursue"
			l_path.replace_substring_all ("{"+"delivery_note_id"+"}", api_client.url_encode (delivery_note_id.out))
