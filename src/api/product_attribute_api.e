...
feature -- API Access
	...
	create_product_attribute (product_attribute_create: PRODUCT_ATTRIBUTE_CREATE): detachable PRODUCT_ATTRIBUTE
			...
			-- 
			-- 
			-- argument: product_attribute_create  (required)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(product_attribute_create)
			l_path := "/api/v1/product-attributes"
			...
			elseif attached { PRODUCT_ATTRIBUTE } l_response.data ({ PRODUCT_ATTRIBUTE }) as l_data then
				Result := l_data
			else
	...
	delete_product_attribute (attribute_id: STRING_32)
			...
			-- 
			-- 
			-- argument: attribute_id  (required)
			-- 
			-- 
		require
		local
		...
		do
			...
			l_path := "/api/v1/product-attributes/{attribute_id}"
			l_path.replace_substring_all ("{"+"attribute_id"+"}", api_client.url_encode (attribute_id.out))
	...
	list_product_attributes (page: INTEGER_32; page_size: INTEGER_32; product_id: UUID; is_filterable: BOOLEAN; search: STRING_32): detachable LIST [PRODUCT_ATTRIBUTE]
			-- 
			-- 
			-- 
			...
			-- 
			-- argument: is_filterable  (optional, default to null)
			-- 
			-- argument: search  (optional, default to null)
			-- 
			-- 
			-- Result LIST [PRODUCT_ATTRIBUTE]
		require
		...
		do
			...
			l_request.fill_query_params(api_client.parameter_to_tuple("", "is_filterable", is_filterable));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [PRODUCT_ATTRIBUTE] } l_response.data ({ LIST [PRODUCT_ATTRIBUTE] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_product_attribute (attribute_id: STRING_32; product_attribute_update: PRODUCT_ATTRIBUTE_UPDATE): detachable PRODUCT_ATTRIBUTE
			...
			-- argument: attribute_id  (required)
			-- 
			-- argument: product_attribute_update  (required)
			-- 
			-- 
			-- Result PRODUCT_ATTRIBUTE
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(product_attribute_update)
			l_path := "/api/v1/product-attributes/{attribute_id}"
			l_path.replace_substring_all ("{"+"attribute_id"+"}", api_client.url_encode (attribute_id.out))
