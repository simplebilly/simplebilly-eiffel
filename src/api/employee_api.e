...
feature -- API Access
	...
	create_employee (employee_create: EMPLOYEE_CREATE): detachable EMPLOYEE
			...
			-- 
			-- 
			-- argument: employee_create  (required)
			-- 
			-- 
			-- Result EMPLOYEE
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(employee_create)
			l_path := "/api/v1/employees"
			...
			elseif attached { EMPLOYEE } l_response.data ({ EMPLOYEE }) as l_data then
				Result := l_data
			else
	...
	employee_payroll_summary (id: UUID; year: INTEGER_32): detachable PAYROLL_SUMMARY
			-- 
			-- 
			-- 
			-- argument: id  (required)
			-- 
			-- argument: year Fiscal year for the breakdown; defaults to the current year. (optional, default to null)
			-- 
			-- 
		...
		do
			...
			l_path := "/api/v1/employees/{id}/payroll-summary"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { PAYROLL_SUMMARY } l_response.data ({ PAYROLL_SUMMARY }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	employee_restore (id: UUID): detachable EMPLOYEE
		...
		do
			...
			l_path := "/api/v1/employees/{id}/restore"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
	...
	employees (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [EMPLOYEE]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [EMPLOYEE] } l_response.data ({ LIST [EMPLOYEE] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_employee (id: UUID; employee_update: EMPLOYEE_UPDATE): detachable EMPLOYEE
			...
			-- argument: id  (required)
			-- 
			-- argument: employee_update  (required)
			-- 
			-- 
			-- Result EMPLOYEE
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(employee_update)
			l_path := "/api/v1/employees/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
