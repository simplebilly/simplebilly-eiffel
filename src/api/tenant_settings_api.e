...
feature -- API Access
	...
	tenant_settings : detachable TENANT_SETTINGS
		...
		do
			...
			elseif attached { TENANT_SETTINGS } l_response.data ({ TENANT_SETTINGS }) as l_data then
				Result := l_data
			else
	...
	update_tenant_settings (update_tenant_settings: UPDATE_TENANT_SETTINGS): detachable TENANT_SETTINGS
			...
			-- 
			-- 
			-- argument: update_tenant_settings  (required)
			-- 
			-- 
			-- Result TENANT_SETTINGS
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(update_tenant_settings)
			l_path := "/api/v1/settings/tenant"
