...
feature -- API Access
	...
	create_silent_partner (silent_partner_create: SILENT_PARTNER_CREATE): detachable SILENT_PARTNER
			...
			-- 
			-- 
			-- argument: silent_partner_create  (required)
			-- 
			-- 
			-- Result SILENT_PARTNER
		require
		local
  			l_path: STRING
		...
		do
			reset_error
			create l_request
			l_request.set_body(silent_partner_create)
			l_path := "/api/v1/silent-partners"
			...
			elseif attached { SILENT_PARTNER } l_response.data ({ SILENT_PARTNER }) as l_data then
				Result := l_data
			else
	...
	delete_silent_partner (id: UUID)
		...
		do
			...
			l_path := "/api/v1/silent-partners/{id}"
	...
	silent_partners (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [SILENT_PARTNER]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [SILENT_PARTNER] } l_response.data ({ LIST [SILENT_PARTNER] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_silent_partner (id: UUID; silent_partner_update: SILENT_PARTNER_UPDATE): detachable SILENT_PARTNER
			...
			-- argument: id  (required)
			-- 
			-- argument: silent_partner_update  (required)
			-- 
			-- 
			-- Result SILENT_PARTNER
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(silent_partner_update)
			l_path := "/api/v1/silent-partners/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
