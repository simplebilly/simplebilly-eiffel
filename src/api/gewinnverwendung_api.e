...
feature -- API Access
	...
	gewinnverwendung_api (year: INTEGER_32): detachable GEWINNVERWENDUNGS_REPORT
			...
			-- 
			-- 
			-- Result GEWINNVERWENDUNGS_REPORT
		require
		local
  			l_path: STRING
		...
		do
			...
			l_path := "/api/v1/bookkeeping/gewinnverwendung"
			...
			elseif attached { GEWINNVERWENDUNGS_REPORT } l_response.data ({ GEWINNVERWENDUNGS_REPORT }) as l_data then
				Result := l_data
			else
	...
	gewinnverwendung_export_api (year: INTEGER_32): detachable GEWINNVERWENDUNGS_EXPORT_RESPONSE
			...
			-- 
			-- 
			-- Result GEWINNVERWENDUNGS_EXPORT_RESPONSE
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			l_path := "/api/v1/bookkeeping/gewinnverwendung/export"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "year", year));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { GEWINNVERWENDUNGS_EXPORT_RESPONSE } l_response.data ({ GEWINNVERWENDUNGS_EXPORT_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
