...
feature -- API Access
	...
	save_smtp_config_api (smtp_config: detachable SMTP_CONFIG): detachable SMTP_CONFIG
		...
		do
			reset_error
			create l_request
			l_request.set_body(smtp_config)
			l_path := "/api/v1/settings/smtp"
			...
			elseif attached { SMTP_CONFIG } l_response.data ({ SMTP_CONFIG }) as l_data then
				Result := l_data
			else
