...
feature -- API Access
	...
	down_payment_invoice (id: STRING_32): detachable DOWN_PAYMENT_INVOICE
		...
		do
			...
			l_path := "/api/v1/down-payment-invoices/{id}"
			...
			elseif attached { DOWN_PAYMENT_INVOICE } l_response.data ({ DOWN_PAYMENT_INVOICE }) as l_data then
				Result := l_data
			else
	...
	download_down_payment_invoice_pdf (id: STRING_32)
		...
		do
			...
			l_path := "/api/v1/down-payment-invoices/{id}/pdf"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
	...
	list_down_payment_invoices (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [DOWN_PAYMENT_INVOICE]
			-- 
			-- 
			-- 
			...
			-- 
			-- 
			-- Result LIST [DOWN_PAYMENT_INVOICE]
		require
		...
		do
			...
			l_path := "/api/v1/down-payment-invoices/"
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [DOWN_PAYMENT_INVOICE] } l_response.data ({ LIST [DOWN_PAYMENT_INVOICE] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
