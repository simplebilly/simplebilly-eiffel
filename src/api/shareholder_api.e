...
feature -- API Access
	...
	create_shareholder (shareholder_create: SHAREHOLDER_CREATE): detachable SHAREHOLDER
			...
			-- 
			-- 
			-- argument: shareholder_create  (required)
			-- 
			-- 
			-- Result SHAREHOLDER
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(shareholder_create)
			l_path := "/api/v1/shareholders"
			...
			elseif attached { SHAREHOLDER } l_response.data ({ SHAREHOLDER }) as l_data then
				Result := l_data
			else
	...
	shareholders (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [SHAREHOLDER]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [SHAREHOLDER] } l_response.data ({ LIST [SHAREHOLDER] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_shareholder (id: UUID; shareholder_update: SHAREHOLDER_UPDATE): detachable SHAREHOLDER
			...
			-- argument: id  (required)
			-- 
			-- argument: shareholder_update  (required)
			-- 
			-- 
			-- Result SHAREHOLDER
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(shareholder_update)
			l_path := "/api/v1/shareholders/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
