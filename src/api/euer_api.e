...
feature -- API Access
	...
	euer_api (year: INTEGER_32): detachable EUER_ERGEBNIS
		...
		do
			...
			l_path := "/api/v1/bookkeeping/euer"
			...
			elseif attached { EUER_ERGEBNIS } l_response.data ({ EUER_ERGEBNIS }) as l_data then
				Result := l_data
			else
	...
	euer_kategorien_api (year: INTEGER_32): detachable EUER_DETAIL_ERGEBNIS
		...
		do
			...
			l_path := "/api/v1/bookkeeping/euer/kategorien"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "year", year));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { EUER_DETAIL_ERGEBNIS } l_response.data ({ EUER_DETAIL_ERGEBNIS }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
