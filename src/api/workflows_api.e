...
feature -- API Access
	...
	list_workflows_api : detachable LIST [WORKFLOW]
		...
		do
			...
			elseif attached { LIST [WORKFLOW] } l_response.data ({ LIST [WORKFLOW] }) as l_data then
				Result := l_data
			else
	...
	set_workflow_enabled_api (workflow_id: STRING_32; workflow_enabled_update: WORKFLOW_ENABLED_UPDATE): detachable WORKFLOW
			...
			-- 
			-- 
			-- argument: workflow_id  (required)
			-- 
			-- argument: workflow_enabled_update  (required)
			-- 
			-- 
			-- Result WORKFLOW
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(workflow_enabled_update)
			l_path := "/api/v1/workflows/{workflow_id}/enabled"
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { WORKFLOW } l_response.data ({ WORKFLOW }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
