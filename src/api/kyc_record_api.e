...
feature -- API Access
	...
	create_kyc_record (kyc_record_create: KYC_RECORD_CREATE): detachable KYC_RECORD
			...
			-- 
			-- 
			-- argument: kyc_record_create  (required)
			-- 
			-- 
			-- Result KYC_RECORD
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(kyc_record_create)
			l_path := "/api/v1/kyc-records"
			...
			elseif attached { KYC_RECORD } l_response.data ({ KYC_RECORD }) as l_data then
				Result := l_data
			else
	...
	kyc_records (page: INTEGER_32; page_size: INTEGER_32; search: STRING_32; include_deleted: BOOLEAN): detachable LIST [KYC_RECORD]
			-- 
			-- 
			-- 
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [KYC_RECORD] } l_response.data ({ LIST [KYC_RECORD] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	update_kyc_record (id: UUID; kyc_record_update: KYC_RECORD_UPDATE): detachable KYC_RECORD
			...
			-- argument: id  (required)
			-- 
			-- argument: kyc_record_update  (required)
			-- 
			-- 
			-- Result KYC_RECORD
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(kyc_record_update)
			l_path := "/api/v1/kyc-records/{id}"
			l_path.replace_substring_all ("{"+"id"+"}", api_client.url_encode (id.out))
