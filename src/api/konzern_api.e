...
feature -- API Access
	...
	konzern_export_api (year: INTEGER_32): detachable KONZERN_EXPORT_RESPONSE
			...
			-- 
			-- 
			-- Result KONZERN_EXPORT_RESPONSE
		require
		local
  			l_path: STRING
		...
		do
			...
			l_path := "/api/v1/bookkeeping/konzern/status/export"
			...
			elseif attached { KONZERN_EXPORT_RESPONSE } l_response.data ({ KONZERN_EXPORT_RESPONSE }) as l_data then
				Result := l_data
			else
	...
	konzern_status_api (year: INTEGER_32): detachable KONZERN_STATUS
		...
		do
			...
			l_path := "/api/v1/bookkeeping/konzern/status"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "year", year));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { KONZERN_STATUS } l_response.data ({ KONZERN_STATUS }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
