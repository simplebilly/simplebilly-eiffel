note
 description:"[
		SimpleBilly API
 		Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
  		The version of the OpenAPI document: 0.1.0
 	    Contact: support@simplebilly.com

  	NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

 		 Do not edit the class manually.
 	]"
	date: "$Date$"
	revision: "$Revision$"
	EIS:"Eiffel openapi generator", "src=https://openapi-generator.tech", "protocol=uri"
class DELIVERY_APPOINTMENT_STATUS

feature -- Access

  value: detachable STRING_32
      -- enumerated value.
    note
            option: stable
    attribute
    end

feature -- Enum

 val_requested: DELIVERY_APPOINTMENT_STATUS
    once
      create Result
      Result.set_value ("requested")
    end

 val_confirmed: DELIVERY_APPOINTMENT_STATUS
    once
      create Result
      Result.set_value ("confirmed")
    end

 val_arrived: DELIVERY_APPOINTMENT_STATUS
    once
      create Result
      Result.set_value ("arrived")
    end

 val_cancelled: DELIVERY_APPOINTMENT_STATUS
    once
      create Result
      Result.set_value ("cancelled")
    end

 val_completed: DELIVERY_APPOINTMENT_STATUS
    once
      create Result
      Result.set_value ("completed")
    end


feature -- Element Change

  set_value (a_val: like value)
      -- Set `value' with `a_value'.
    do
      value := a_val
    ensure
      value_set: value = a_val
    end


end

